    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            try
            {
                ...
                StreamReader srServer = new StreamReader("serverName");
                StreamReader srSql = new StreamReader("sqlDB");
                path[0] = sr.ReadLine();
                path[1] = srf.ReadLine();
                sr.Close();
                srf.Close();
                ...
                tbPath.Text = path[0];
                tbFreq.Text = path[1];
                path[2] = srServer.ReadLine();
                tbServerName.Text = path[2];
                srServer.Close();
                srServer.Dispose();
                path[5] = srSql.ReadLine();
                tbBaseName.Text = path[5];
            }
            catch(Exception ex)
            {
            ...
            try
            {
                StreamReader srlogin = new StreamReader("login");
                StreamReader srpassword = new StreamReader("password");
                path[3] = srlogin.ReadLine();
                if(path[3] == null)
                {
                    path[3] = "nologin";
                }
            ...
            try
            {
                ...
                StreamReader srLocalDB = new StreamReader("isLocalDB");
                ...
                localbd = Convert.ToBoolean(srLocalDB.ReadLine());
                srTray.Close();
                ...
                cbLocalBd.IsChecked = localbd;
                tbLogin.IsEnabled = !localbd;
                tbPassword.IsEnabled = !localbd;
                path[6] = localbd.ToString();
                srLocalDB.Close();
                srLocalDB.Dispose();
            }
            catch
            ...
            openFile.FileName = "../DataService/DataService.exe";
        ...
        private void btnApplySql_Click(object sender, RoutedEventArgs e)
        {
            ServiceController service = new ServiceController("Служба интеграции");
            if (service.Status != ServiceControllerStatus.Running)
            {
                try
                {
                    path[2] = tbServerName.Text;
                    path[3] = tbLogin.Text;
                    path[4] = tbPassword.Text;
                    path[5] = tbBaseName.Text;
                    path[6] = cbLocalBd.IsChecked.ToString();
                    ...
                    dataTables.Clear();
                    indexes.Clear();
                    File.WriteAllText("serverName", path[2]);
                    File.WriteAllText("login", path[3]);
                    File.WriteAllText("password", path[4]);
                    File.WriteAllText("sqlDB", path[5]);
                    File.WriteAllText("isLocalDB", path[6]);
                    StopService("Служба интеграции");
                    GC.Collect();
                    txtCurrentState.Text = "Текущее состояние службы:\n Служба приостановлена";
                    txtCurrentState.Foreground = Brushes.Yellow;
            ...
            {
                if (MessageBox.Show("Чтобы выбрать настройки сервера, необходимо остановить службу. Остановить службу и поменять настройки?", "Предупреждение", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                {
                    try
                    {
                        path[2] = tbServerName.Text;
